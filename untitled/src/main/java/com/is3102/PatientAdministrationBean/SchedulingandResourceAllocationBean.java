...
Doctor doctor;
    ...
    public void addDoctor(String id, String name, String username, String dob) throws ExistException{

        doctor = em.find(Doctor.class, id);
        if (doctor != null)
            throw new ExistException("DOCTOR ALREADY EXISTS.");
        doctor =  new Doctor();
        doctor.create(new Long(id), name, username, dob);
        em.persist(doctor);
